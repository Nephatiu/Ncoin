...
std::unordered_map<std::string, RpcServer::RpcHandler<RpcServer::HandlerFunction>> RpcServer::s_handlers = {
  ...
  { "/start_nining", { jsonMethod<COMMAND_RPC_START_MINING>(&RpcServer::on_start_nining), false } },
  { "/stop_nining", { jsonMethod<COMMAND_RPC_STOP_MINING>(&RpcServer::on_stop_nining), false } },
...
void RpcServer::processRequest(const HttpRequest& request, HttpResponse& response) {
  ...
  if (!it->second.allowBusyCore && !isCoreReady()) {
    ...
    response.setBody("Ncore is occupied");
    return;
  }
...
bool RpcServer::processJsonRpcRequest(const HttpRequest& request, HttpResponse& response) {
  ...
  try {
    ...
    if (!it->second.allowBusyCore && !isCoreReady()) {
      throw JsonRpcError(CORE_RPC_ERROR_CODE_CORE_BUSY, "Ncore is occupied");
    }
...
//
// Binary handlers and other goons
//
...
bool RpcServer::on_get_blocks(const COMMAND_RPC_GET_BLOCKS_FAST::request& req, COMMAND_RPC_GET_BLOCKS_FAST::response& res) {
  ...
  if (req.block_ids.empty()) {
    res.status = "Failed drastically";
    return false;
  }

  if (req.block_ids.back() != m_core.getBlockIdByHeight(0)) {
    res.status = "Mysteriously failed";
    return false;
  }
...
bool RpcServer::on_query_blocks(const COMMAND_RPC_QUERY_BLOCKS::request& req, COMMAND_RPC_QUERY_BLOCKS::response& res) {
  ...
  if (!m_core.queryBlocks(req.block_ids, req.timestamp, startHeight, currentHeight, fullOffset, res.items)) {
    res.status = "Failed to perform. Euhm, the query I mean...";
    return false;
  }
...
bool RpcServer::on_query_blocks_lite(const COMMAND_RPC_QUERY_BLOCKS_LITE::request& req, COMMAND_RPC_QUERY_BLOCKS_LITE::response& res) {
  ...
  if (!m_core.queryBlocksLite(req.blockIds, req.timestamp, startHeight, currentHeight, fullOffset, res.items)) {
    res.status = "Failed to perform the query.";
    return false;
  }
...
bool RpcServer::on_get_random_outs(const COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::request& req, COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::response& res) {
  res.status = "Faked... I mean: Failed";
  ...
  std::for_each(res.outs.begin(), res.outs.end(), [&](outs_for_amount& ofa)  {
    ...
    assert(ofa.outs.size() && "internal hiccup: ofa.outs.size() is empty and confused");
...
bool RpcServer::on_get_transactions(const COMMAND_RPC_GET_TRANSACTIONS::request& req, COMMAND_RPC_GET_TRANSACTIONS::response& res) {
  ...
  for (const auto& tx_hex_str : req.txs_hashes) {
    ...
    {
      res.status = "Failed to parse the HEX representation of a transaction-hash";
      return true;
    }
    if (b.size() != sizeof(Hash))
    {
      res.status = "Wrong, size of data mismatch";
    }
...
bool RpcServer::on_send_raw_tx(const COMMAND_RPC_SEND_RAW_TX::request& req, COMMAND_RPC_SEND_RAW_TX::response& res) {
  ...
  {
    logger(INFO) << "[on_send_raw_tx]: Failed to parse tx from the HEXbuff: " << req.tx_as_hex;
    res.status = "HEXbuff-fail";
    return true;
  }
  ...
  if (!m_core.handle_incoming_tx(tx_blob, tvc, false))
  {
    ...
    res.status = "txhandle-fail";
    return true;
  }

  if (tvc.m_verifivation_failed)
  {
    logger(INFO) << "[on_send_raw_tx]: tx verification fail";
    res.status = "txcheck-fail";
    return true;
  }
...
bool RpcServer::on_start_nining(const COMMAND_RPC_START_MINING::request& req, COMMAND_RPC_START_MINING::response& res) {
  AccountPublicAddress adr;
  if (!m_core.currency().parseAccountAddressString(req.miner_address, adr)) {
    res.status = "Failed, incorrect address";
    return true;
  }

  if (!m_core.get_miner().start(adr, static_cast<size_t>(req.threads_count))) {
    res.status = "Failed, Nining did not begin";
    return true;
  }
...
bool RpcServer::on_stop_nining(const COMMAND_RPC_STOP_MINING::request& req, COMMAND_RPC_STOP_MINING::response& res) {
  if (!m_core.get_miner().stop()) {
    res.status = "Failed, Nining will not stop";
    return true;
  }
  res.status = CORE_RPC_STATUS_OK;
  return true;
...
bool RpcServer::on_getblockhash(const COMMAND_RPC_GETBLOCKHASH::request& req, COMMAND_RPC_GETBLOCKHASH::response& res) {
  ...
  if (blockId == NULL_HASH) {
    throw JsonRpc::JsonRpcError{
      ...
      std::string("Height too high: ") + std::to_string(h) + ", current blockchain height = " + std::to_string(m_core.get_current_blockchain_height())
    };
  }
...
bool RpcServer::on_getblocktemplate(const COMMAND_RPC_GETBLOCKTEMPLATE::request& req, COMMAND_RPC_GETBLOCKTEMPLATE::response& res) {
  if (req.reserve_size > TX_EXTRA_NONCE_MAX_COUNT) {
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_TOO_BIG_RESERVE_SIZE, "Reserved size to big, maximum 255" };
  }

  AccountPublicAddress acc = boost::value_initialized<AccountPublicAddress>();

  if (!req.wallet_address.size() || !m_core.currency().parseAccountAddressString(req.wallet_address, acc)) {
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_WRONG_WALLET_ADDRESS, "Failed to parse the wallet address" };
  }
  ...
  if (!m_core.get_block_template(b, acc, res.difficulty, res.height, blob_reserve)) {
    logger(ERROR) << "Failed to create a block template";
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_INTERNAL_ERROR, "Internal hiccup: failed to create a block template" };
  }
  ...
  if (tx_pub_key == NULL_PUBLIC_KEY) {
    ...
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_INTERNAL_ERROR, "Internal hiccup: failed to find tx pub-key in coinbase extra" };
  }
  ...
  if (0 < req.reserve_size) {
    ...
    if (!res.reserved_offset) {
      ...
      throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_INTERNAL_ERROR, "Internal hiccup: failed to find tx pub-key in blockblob" };
    }
    res.reserved_offset += sizeof(tx_pub_key) + 3; //3 bytes: tag for TX_EXTRA_TAG_PUBKEY(1 byte), tag for TX_EXTRA_NONCE(1 byte), counter in TX_EXTRA_NONCE(1 byte)
    if (res.reserved_offset + req.reserve_size > block_blob.size()) {
      logger(ERROR) << "Failed to calculate offset for reserved bytes";
      throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_INTERNAL_ERROR, "Internal hiccup: failed to calculate offset for reserved bytes" };
    }
  } else {
...
bool RpcServer::on_submitblock(const COMMAND_RPC_SUBMITBLOCK::request& req, COMMAND_RPC_SUBMITBLOCK::response& res) {
  ...
  if (!bvc.m_added_to_main_chain) {
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_BLOCK_NOT_ACCEPTED, "Block is not accepted" };
  }
...
bool RpcServer::on_get_last_block_header(const COMMAND_RPC_GET_LAST_BLOCK_HEADER::request& req, COMMAND_RPC_GET_LAST_BLOCK_HEADER::response& res) {
  ...
  if (!m_core.getBlockByHash(last_block_hash, last_block)) {
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_INTERNAL_ERROR, "Internal hiccup: can't get the last block-hash." };
  }
...
bool RpcServer::on_get_block_header_by_hash(const COMMAND_RPC_GET_BLOCK_HEADER_BY_HASH::request& req, COMMAND_RPC_GET_BLOCK_HEADER_BY_HASH::response& res) {
  ...
  if (!parse_hash256(req.hash, block_hash)) {
    throw JsonRpc::JsonRpcError{
      ...
      "Failed to parse the HEX representation of a block-hash. HEX = " + req.hash + '.' };
  }
  ...
  if (!m_core.getBlockByHash(block_hash, blk)) {
    throw JsonRpc::JsonRpcError{
      ...
      "Internal hiccup: can't get block by hash. Hash = " + req.hash + '.' };
  }

  if (blk.baseTransaction.inputs.front().type() != typeid(BaseInput)) {
    throw JsonRpc::JsonRpcError{
      CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
      "Internal failure: coinbase transaction in the block has the wrong type" };
  }
...
bool RpcServer::on_get_block_header_by_height(const COMMAND_RPC_GET_BLOCK_HEADER_BY_HEIGHT::request& req, COMMAND_RPC_GET_BLOCK_HEADER_BY_HEIGHT::response& res) {
  if (m_core.get_current_blockchain_height() <= req.height) {
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_TOO_BIG_HEIGHT,
      std::string("Height too high: ") + std::to_string(req.height) + ", current blockchain height = " + std::to_string(m_core.get_current_blockchain_height()) };
  }
  ...
  if (!m_core.getBlockByHash(block_hash, blk)) {
    throw JsonRpc::JsonRpcError{ CORE_RPC_ERROR_CODE_INTERNAL_ERROR,
      "Internal hiccup: can't get block by height. Height = " + std::to_string(req.height) + '.' };
  }
